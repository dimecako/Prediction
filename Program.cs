...
public class ProductionConsensusAggregator
{
	...
	public async Task<List<SiteMatch>> ParseStatareaAsync(IBrowser browser)
	{
		...
		// 1. Користиме FlareSolverr наместо ScrapeHtmlAsync за да ја пробиеме заштитата
		// Додаваме и "/starttime" на крајот за правилна редирекција
		string url = $"https://www.statarea.com/predictions/date/{targetDate}/";
		var soup = await GetPageFromFlareSolverr(url);
		
		if (soup == null) return matches;

		var headerRow = soup.DocumentNode.SelectSingleNode("//tr[contains(@class, 'had') or contains(@class, 'header')]");
		Dictionary<string, int> columnIndices = new Dictionary<string, int> { { "1", 3 }, { "X", 4 }, { "2", 5 }, { "btts", 11 } };

		if (headerRow != null)
		{
			var headers = headerRow.SelectNodes(".//td | .//th")?.Select(n => n.InnerText.Trim().ToLower()).ToList();
			if (headers != null)
			{
				var keys = new[] { ("1", "1"), ("X", "x"), ("2", "2"), ("btts", "btts") };
				foreach (var k in keys) { if (headers.Contains(k.Item2)) columnIndices[k.Item1] = headers.IndexOf(k.Item2); }
			}
		}

		var matchRows = soup.DocumentNode.SelectNodes("//div[@class='match']");
		if (matchRows != null)
		{
			foreach (var row in matchRows)
			{
				try
				{
					var homeNode = row.SelectSingleNode(".//div[@class='hostteam']//div[@class='name']/a");
					var awayNode = row.SelectSingleNode(".//div[@class='guestteam']//div[@class='name']/a");
					if (homeNode == null || awayNode == null) continue;

					string home = homeNode.InnerText.Trim();
					string away = awayNode.InnerText.Trim();

					var values = row.SelectNodes(".//*[contains(@class, 'value')] | .//td[contains(@class, 'value')]");
					if (values == null || values.Count <= columnIndices.Values.Max()) continue;

					// 2. Безбедно парсирање (TryParse) за да не скока во 'catch' ако вредноста е празна поради Cloudflare
					if (!int.TryParse(values[columnIndices["1"]].InnerText.Replace("%", "").Trim(), out int p1)) continue;
					if (!int.TryParse(values[columnIndices["X"]].InnerText.Replace("%", "").Trim(), out int pX)) continue;
					if (!int.TryParse(values[columnIndices["2"]].InnerText.Replace("%", "").Trim(), out int p2)) continue;
					
					string bttsYes = values.Count > columnIndices["btts"] ? values[columnIndices["btts"]].InnerText.Trim() : "-";

					int maxP = Math.Max(p1, Math.Max(pX, p2));
					string tip = maxP == p1 ? "1" : (maxP == pX ? "X" : "2");
					...
					matches.Add(new SiteMatch
					{
						...
						Prob = maxP,
						Btts = bttsYes,
						BttsMarket = int.TryParse(bttsYes, out int b) ? (b >= 50 ? "YES" : "NO") : "-"
					});
				}
				catch 
				{ 
					// Во случај на неочекувана грешка, продолжуваме со следниот натпревар
					continue; 
				}
			}
		}
